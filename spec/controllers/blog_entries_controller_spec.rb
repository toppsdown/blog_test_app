...
describe BlogEntriesController, :type => :controller do
  ...
  describe 'POST #create' do
    ...
    context 'extra params need to be filtered' do
      ...
      it 'whitelists expected parameters' do
        expect(BlogEntry).to receive(:new).with(hash_including(*expected_params)).and_return(blog_entry)
    ...
    context 'on successful save' do
      it 'saves new blog entry' do
        ...
        existing_entries = BlogEntry.all
        expect(existing_entries.count).to eq(1)
        existing_entries.first.tap do |entry|
